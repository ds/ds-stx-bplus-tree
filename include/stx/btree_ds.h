...
//This conflicts with postgres source
...
#include "common/include/circular.h"
#include "common/include/walker_callback.h"
#include "common/include/string_utils.h"

#include "qe/common/data_page_allocator.h"
#include "qe/common/available_page_mgr.h"
#include "qe/common/btree_page_header.h"

#define ENABLE_FIXUP
// define USE_DATA_PAGES to enable use of DS data pages as the store for btree
// nodes.  If not defined, then nodes are allocated in the heap.
#define USE_DATA_PAGES
// *** Debugging Macros
//#define BTREE_DEBUG
#ifdef BTREE_DEBUG
...
#define ASSIGN_CHILD(base, idx, v) { base->childid[idx] = v;\
                                     base->childid[idx]->parent = base;}

/// STX - Some Template Extensions namespace
namespace stx {
    /// iterator base class to provide support for adjustment behavior.
    class iter_call_back
    {
    public:
        typedef void leaf_node;

        virtual ~iter_call_back() {}

        /// when btree becomes empty, invalidate all iterators
        virtual void invalidateIterators() {}

        /// two leaf nodes merged after deleting slot in left node
        virtual void merge_left(leaf_node* left, leaf_node* right, unsigned short slot,
                                unsigned short leftsize) {}

        /// two leaf nodes merged after deleting slot in right node
        virtual void merge_right(leaf_node* left, leaf_node* right, unsigned short slot,
                                unsigned short leftsize) {}

        /// origNode split into two after inserting into slot.  Split at j
        virtual void split(leaf_node* orignode, leaf_node* newnode,
                           unsigned short slot, unsigned short splitpoint)
        {
            printf("entered base split\n"); 
        }

        /// shift count nodes from right to left after insert at slot
        /// shifting is always from right to left
        virtual void shift_left(leaf_node* left, leaf_node* right, unsigned short slot,
                                unsigned short leftsize, unsigned short count)
        {
            printf("entered base shift_left\n");
        }

        /// shift count nodes from right to left after insert at slot
        /// shifting is alwarys from left to right
        virtual void shift_right(leaf_node* left, leaf_node* right, unsigned short slot,
                                 unsigned short leftsize, unsigned short count)
        {
            printf("entered base shift_right\n");
        }
        // simple insert with no node splitting
        virtual void simple_insert(leaf_node* leaf, unsigned short slot)
        {
            printf("entered base simple_insert\n");
        }
        // simple erase with no node merging or shifting
        virtual void simple_erase(leaf_node* leaf, unsigned short slot)
        {
            printf("entered base simple_erase\n");
        }
    };


typedef boost::unordered_set<BtreePageHeader*> PageSetT;
...
#define sizeOfNode (sizeof(leaf_node) > sizeof(inner_node) ? sizeof(leaf_node) : sizeof(inner_node))
private:
    // *** page level allocator
    
    friend class AvailablePageMgr<BtreePageHeader>;

public:
    void* getGlobalLastLeafModified() {
        return this->globalLastLeafModified_;
    }
private:

    BtreePageHeader * allocatePage() {
        BtreePageHeader *bphp =  new(shard_->alloc(true))
                                BtreePageHeader(this->entriesPerPage());
        ps_->insert(bphp);
        return bphp;
    }

    void releasePage(BtreePageHeader *bphp) {
        ps_->erase(bphp);
        shard_->free(bphp);
    }

    virtual uint16_t entriesPerPage() {
        return (DataMemoryPageSize - sizeof(BtreePageHeader))
			/ sizeOfNode;
    }
    
    virtual uint16_t rowsRemainingInPage(DataPageHeader *php) {
        BtreePageHeader *bphp = static_cast<BtreePageHeader*>(php);
        return bphp->getRemainingRows();
    }

    virtual uint32_t getMaxRemainingRowsPerPage() {
...
public:
    ...
    {
    //protected:
    //RBR! this is requierd to support the btree_adapter_impl
    public:
        // *** Members
    ...
    public:
        ...
        inline base_iterator(typename btree::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }

        // Support updating of concurrent iterators

        /// two leaf nodes merged after deleting slot in left node
        void merge_left(leaf_node* left, leaf_node* right,
                        unsigned short slot, unsigned short leftsize)
        {
            if (currnode != left && currnode != right) {
                // most likely state of affairs.  iterator is not
        ...
        /// two leaf nodes merged after deleting slot in right node
        void merge_right(leaf_node* left, leaf_node* right, unsigned short slot,
                         unsigned short leftsize)
        {
            if (currnode != left && currnode != right) {
                // most likely state of affairs.  iterator is not
            ...
            if (currnode == right) {
                if (currslot == slot && (left->slotuse - leftsize) == slot) {
                    // iterator was on last element of right
                    // element removed was last element of right
                    // Iterator is either at end of btree of must
                    // move to next node
                    if (left->nextleaf != NULL) {
                        currnode = left->nextleaf;
                        currslot = 0;
                    } else {
                        // at end()
                        currnode = left;
                        currslot = left->slotuse;
                    }
                    return;
                }
                currnode = left;
                currslot += leftsize - (currslot > slot ? 1 : 0);
            }
        }


        /// origNode split into two after inserting into slot.  Split at j
        void split(leaf_node* orignode, leaf_node* newnode,
                   unsigned short slot, unsigned short splitpoint)
        {
            if (currnode != orignode) {
                return;
            }
            unsigned short c = currslot;
            if (c >= splitpoint) {
                currnode = newnode;
                currslot = c - splitpoint + (c > slot && slot >= splitpoint ? 1 : 0);
            } else {
                currslot = c + (c > slot ? 1 : 0);
            }
        }

        /// shift count nodes from right to left after insert at slot
        /// shifting is always from right to left
        void shift_left(leaf_node* left, leaf_node* right, unsigned short slot,
                        unsigned short leftsize, unsigned short count)
        {
            if (currnode != left && currnode != right) {
                return;
            ...
            if (currnode == left) {
                ...
                if (currslot < count) {
                    currnode = left;
                    currslot += leftsize; // should this be decremented ??
                } else {
                    currslot -= count;
                }
            }
        ...
        /// shift count nodes from right to left after insert at slot
        /// shifting is alwarys from left to right
        void shift_right(leaf_node* left, leaf_node* right, unsigned short slot,
                         unsigned short leftsize, unsigned short count)
        {
            if (currnode != left && currnode != right) {
                return;
            ...
            if (currnode == right) {
                if ((currslot + count) >= right->slotuse && currslot == slot) {
                    // element we were at was removed and it was at the end of
                    // the node.  
                    if (right->nextleaf != NULL) {
                        // can move to next node.  Do so
                        currnode = right->nextleaf;
                        currslot = 0;
                    } else {
                        // at end().  Leave iterator at there
                        return;
                    }
                } else {
                    currslot += count + (currslot > slot ? -1 : 0);
                }
            } else if (currslot >= leftsize - count) {
                currnode = right;
                currslot -= (leftsize - count);
            }
        }

        // simple insert with no node splitting
        void simple_insert(leaf_node* leaf, unsigned short slot)
        {
            if (currnode == leaf) {
                if (currslot > slot) {
                    ++currslot;
        ...
        // simple erase with no node merging or shifting
        void simple_erase(leaf_node* leaf, unsigned short slot)
        {
            if (currnode == leaf) {
                if (currslot > slot) {
                    --currslot;
                } else if (currslot == slot && slot == leaf->slotuse) {
                    // at end of node, need to move to next node or ...
                    if (leaf->nextleaf != NULL) {
                        currnode = leaf->nextleaf;
                        currslot = 0;
                    } else {
                        // iterator is at end.  
                        // nothing to do
    ...
    //private:
        ...
        /// Also friendly to btree to permit access for erase(iterator) functions
    ...
    public:
...
#else
            : base_iterator(it.currnode, it.currslot)
#endif
        { }
        ...
        {
            temp_value = pair_to_value_type()( pair_type(this->currnode->slotkey[this->currslot],
                                                         this->currnode->slotdata[this->currslot]) );
            return temp_value;
        }
        ...
        {
            return this->currnode->slotkey[this->currslot];
        }

        /// Writable reference to the current data object
        inline data_type& data() const
        {
            return this->currnode->slotdata[this->currslot];
        }

        /// Prefix++ advance the iterator to the next slot
        inline self& operator++()
        {
            if (this->currslot + 1 < this->currnode->slotuse) {
                ++this->currslot;
            }
            else if (this->currnode->nextleaf != NULL) {
                this->currnode = this->currnode->nextleaf;
                this->currslot = 0;
            }
            else {
                // this is end()
                this->currslot = this->currnode->slotuse;
            }
        ...
        inline self& operator--()
        {
            ...
            else if (this->currnode->prevleaf != NULL) {
                this->currnode = this->currnode->prevleaf;
                this->currslot = this->currnode->slotuse - 1;
            }
            else {
                // this is begin()
                this->currslot = 0;
        ...
        {
            return (x.currnode == this->currnode) && (x.currslot == this->currslot);
        }
    ...
    public:
        ...
        inline reference operator*() const
        {
            BTREE_ASSERT(this->currslot > 0);
            temp_value = pair_to_value_type()( pair_type(this->currnode->slotkey[this->currslot - 1],
                                                         this->currnode->slotdata[this->currslot - 1]) );
            return temp_value;
        }
        ...
        /// Key of the this->current slot
        inline const key_type& key() const
        {
            BTREE_ASSERT(this->currslot > 0);
            return this->currnode->slotkey[this->currslot - 1];
        }

        /// Writable reference to the this->current data object
        inline data_type& data() const
        {
            BTREE_ASSERT(this->currslot > 0);
            return this->currnode->slotdata[this->currslot - 1];
        }
        ...
        inline self& operator--()
        {
            if (this->currslot < this->currnode->slotuse) {
                ++this->currslot;
            }
            else if (this->currnode->nextleaf != NULL) {
                this->currnode = this->currnode->nextleaf;
    ...
    public:
        ...
        /// Read-only reference to the this->current data object
        inline const data_type& data() const
        {
            BTREE_ASSERT(this->currslot > 0);
            return this->currnode->slotdata[this->currslot - 1];
...
private:
    ...
    // Shard in which btree in allocated
    AbstractShard* shard_;

    // Cache context for shard
    const ShardContext ctx_;

    // iterator fixup callback
    iter_call_back* icb_;

    // context for callbacks
    void *context_;

    /// Debug output key func definition
    typedef void (*IndexPrintKeyFunc)(FILE* f, const ShardContext& ctx, const key_type& key);
    IndexPrintKeyFunc       printKeyFunc_; 

    leaf_node* globalLastLeafModified_;

    AvailablePageMgr<BtreePageHeader> *pgMgr_;
    PageSetT *ps_;
    bool monitor_;
...
public:
    ...
    inline btree()
        : root(NULL), headleaf(NULL), tailleaf(NULL),
          shard_(NULL), icb_(NULL), context_(NULL)
    {
        pgMgr_ = NULL;
        ps_ = NULL;
        monitor_ = false;
    ...
    inline btree(const key_compare &kcf)
        : root(NULL), headleaf(NULL), tailleaf(NULL),
          key_less(kcf), shard_(NULL), icb_(NULL), context_(NULL)
    {
        pgMgr_ = NULL;
        ps_ = NULL;
        monitor_ = false;
    ...
    /// constructor initializing an empy B+ tree with special key
    /// comparison object and a context for that object
    /*inline*/ btree(void* context, const key_compare &kcf)
        : root(NULL), headleaf(NULL), tailleaf(NULL),
          key_less(kcf), shard_(NULL), icb_((iter_call_back*)context), context_(context)
    {
        pgMgr_ = new AvailablePageMgr<BtreePageHeader>(this, shard_, sizeOfNode);
        ps_ = new PageSetT(1000);
        monitor_ = false;
    }

    /// constructor initializing an empy B+ tree with special key
    /// comparison object and a context for that object
    /*inline*/ btree(void * context, const key_compare &kcf, AbstractShard* shard)
        : root(NULL), headleaf(NULL), tailleaf(NULL),
          key_less(kcf), shard_(shard), ctx_(shard, 0/*xid*/), icb_((iter_call_back*)context), context_(context)
    {
        pgMgr_ = new AvailablePageMgr<BtreePageHeader>(this, shard_, sizeOfNode);
        ps_ = new PageSetT(1000);
        monitor_ = false;
    ...
    /// constructor initializing an empy B+ tree with special key
    /// comparison object and a context for that object
    /*inline*/ btree(void * context,
                     const key_compare& kcf,
                     AbstractShard* shard,
                     IndexPrintKeyFunc printKeyFunc)
        : root(NULL), headleaf(NULL), tailleaf(NULL),
          key_less(kcf), shard_(shard), ctx_(shard, 0/*xid*/), icb_((iter_call_back*)context), context_(context), printKeyFunc_(printKeyFunc)
    {
        pgMgr_ = new AvailablePageMgr<BtreePageHeader>(this, shard_, sizeOfNode);
        ps_ = new PageSetT(1000);
        monitor_ = false;
    ...
    /// FIXME the following does not work with the DataMemory
    ///       allocation model.  I suggest that we simply disable
    ///       this function.
...
private:
    ...
    {
        return !key_less(ctx_, a, b) && !key_less(ctx_, b, a);
    }

private:
    // *** Node Object Allocation and Deallocation Functions
    inline node *allocateNode(inner_node* parent) {
        BtreePageHeader *bphp = reinterpret_cast<BtreePageHeader*>
            (pgMgr_->findAvailablePage(1));
        node *nn = reinterpret_cast<node*>(bphp->allocateRow(sizeOfNode));
        pgMgr_->addPage(bphp);
        nn->parent = parent;
        return nn;
    }
    ...
    inline leaf_node* allocate_leaf(inner_node* parent)
    {
#ifndef USE_DATA_PAGES
        leaf_node* n = new leaf_node;
#else
        leaf_node* n = reinterpret_cast<leaf_node*>(allocateNode(parent));
#endif
        n->initialize();
        stats.leaves++;
        return n;
    ...
    inline inner_node* allocate_inner(unsigned short l, inner_node* parent)
    {
#ifndef USE_DATA_PAGES
        inner_node* n = new inner_node;
#else
        inner_node* n = reinterpret_cast<inner_node*>(allocateNode(parent));
#endif
        n->initialize(l);
        stats.innernodes++;
        return n;
...
#ifndef USE_DATA_PAGES
     if (n->isleafnode()) {
            memset(n, 0xbe, sizeof(leaf_node));
            delete static_cast<leaf_node*>(n);
            stats.leaves--;
        }
        else {
            memset(n, 0xbe, sizeof(inner_node));
        ...
        // now, release the node back to its container
        memset(n, 0xbe, sizeof(leaf_node));
        BtreePageHeader *php = BTNODE_ADDR_TO_BT_PAGE_ADDR(n); 
        php->releaseNode(n);
        if (rowsRemainingInPage(php) >= getMaxRemainingRowsPerPage()) {
            pgMgr_->removePage(php);
            releasePage(php);
        } else {
            pgMgr_->addPage(php);
        }
...
protected:
    ...
    typedef std::set<std::string> nstrm;
    // the following permits keys to be dumped when
    // examining trees.  This is HIGHLY dependent upon the
    // rows being analyzed.  IT IS NOT GENERAL
    void writeHeader(FILE* f, const char* label) {
        fprintf(f, "digraph %s {\n", label);
        fprintf(f, "\tnode [shape=record];\n");
    }

    void writeTrailer(FILE* f) {
        fprintf(f, "}\n");
    }

    void addConnection(nstrm& connections, node* curr) {
        std::stringstream buf;

        if (curr == root) {
            return;
        }
        inner_node* parent = curr->parent;
        uint16_t slot = get_parent_slot(curr);
        buf << boost::format("\tn%lx:c%d -> n%lx:k0;\n")
                % (uint64_t)parent % slot % (uint64_t)curr;
        connections.insert(buf.str());

    }

    void writeConnections(FILE* f, nstrm& connections) {
        BOOST_FOREACH(std::string s, connections) {
            fputs(s.c_str(), f);
        }
    }

    void writeNodePrefix(FILE* f, node* curr) {
        fprintf(f, "\tn%lx [label=\"", (uint64_t)curr);
    }

    void writeInnerNode(FILE* f, inner_node* inner) {
        writeNodePrefix(f, (node*)inner);
        fprintf(f, "{{<k_1>L%d|",inner->level);
        for (int i = 0; i < inner->slotuse; ++i) {
            fprintf(f, "<k%d> ", i);
            printKeyFunc_(f, ctx_, inner->slotkey[i]);
            fprintf(f, "|");
        }
        fprintf(f, "}|{");
        for (int i = 0; i <= inner->slotuse; ++i) {
            fprintf(f, "<c%d>%s", i, i == inner->slotuse ? "" : "|");
        }
        fprintf(f, "}}\"];\n");
    }

    void writeNextLayer(FILE* f, node* child,
                               nstrm& connections) {
        if (child == root) {
          return;
        }

        inner_node* curr = child->parent;
        uint16_t parentSlot = get_parent_slot(curr);
        inner_node* left = find_left(curr, parentSlot);
        inner_node* right = find_right(curr, parentSlot);
        if (left != NULL) {
            writeInnerNode(f, left);
            addConnection(connections, left);
        }
        writeInnerNode(f, curr);
        addConnection(connections, curr);

        if (right != NULL) {
            writeInnerNode(f, right);
            addConnection(connections, right);
        }
      
        writeNextLayer(f, curr, connections); 
    }

    void writeLeafNode(FILE* f, leaf_node* leaf) {
        writeNodePrefix(f, (node*)leaf);    
        for (int i = 0; i < leaf->slotuse; ++i) {
            fprintf(f, "<k%d> ", i);
            printKeyFunc_(f, ctx_, leaf->slotkey[i]);
            fprintf(f, "%s", i+1 < leaf->slotuse ? "|" : " ");
        }
        fprintf(f, "\"];\n");
    }
...
public:
    // monitor setter for dynamic debugging
    void setMonitor(bool value) {
        this->monitor_ = value;
    }

    // publishTreeSlice outputs a readable slice of a tree, inverted
    void publishTreeSlice(iterator& iter) {
        publishTreeSlice(iter.currnode);
    }

    void publishTreeSlice(inner_node* curr) {
        if (curr == NULL) {
            return;
        }
        ...
        writeConnections(f, connections);
    ...
    void publishTreeSlice(leaf_node* curr) {
        nstrm connections;
        ...
        if (curr->prevleaf != NULL) {
            writeLeafNode(f, curr->prevleaf);
            addConnection(connections, curr->prevleaf);
        }

        writeLeafNode(f, curr);
        addConnection(connections, curr);

        if (curr->nextleaf != NULL) {
            writeLeafNode(f, curr->nextleaf);
            addConnection(connections, curr->nextleaf);
        }
...
#else
        if (root) {
            BOOST_FOREACH(BtreePageHeader *php, *ps_) {
                shard_->free(php);
            }
            ps_->erase(ps_->begin(), ps_->end());
            root->parent = NULL;
        }
#endif        
    }
...
private:
    ...
    inline int find_lower(const node_type *n, const key_type& key) const
    {
        ...
        if (hi < 0 || key_less(ctx_, n->slotkey[hi], key))
            hi++;
    ...
    inline int find_upper(const node_type *n, const key_type& key) const
    {
        ...
        while(lo < hi)
        {
            ...
            if (key_less(ctx_, key, n->slotkey[mid])) {
                hi = mid - 1;
            }
            else {
                lo = mid + 1;
        ...
        {
            int i = n->slotuse - 1;
            while(i >= 0 && key_less(ctx_, key, n->slotkey[i]))
                i--;
            i++;
    ...
    inline btree_self& operator= (const btree_self &other)
    {
        if (this != &other)
        {
            ...
            if (other.size() != 0)
            {
                ...
                if (other.root) {
                    root = copy_recursive(other.root, NULL);
                    root->parent = NULL;
                }
                stats = other.stats;
            }
...
private:
    ...
    struct node* copy_recursive(const node *n, node* newparent)
    {
        if (n->isleafnode())
        {
            const leaf_node *leaf = static_cast<const leaf_node*>(n);
            leaf_node *newleaf = allocate_leaf(newparent);
        ...
        {
            const inner_node *inner = static_cast<const inner_node*>(n);
            inner_node *newinner = allocate_inner(inner->level, newparent);
            ...
            {
                ASSIGN_CHILD(newinner, slot, copy_recursive(inner->childid[slot],newinner));
            }
...
public:
    ...
    /// non-template function cannot be called. If the tree does not allow
    /// duplicate keys, then the insert may fail if it is already present.
    inline bool insert3(const key_type& key, const data_type& data)
    {
        return insert_start(key, data).second;
    }
...
private:
    ...
    std::pair<iterator, bool> insert_start(const key_type& key, const data_type& value)
    {
        ...
        if (root == NULL) {
            root = headleaf = tailleaf = allocate_leaf(NULL);
            root->parent = NULL;
        }
        ...
        if (newchild)
        {
            inner_node *newroot = allocate_inner(root->level + 1, NULL);
            newroot->slotkey[0] = newkey;

            ASSIGN_CHILD(newroot, 0, root);
            ASSIGN_CHILD(newroot, 1, newchild);
    ...
    */
                                             ...
                                             key_type* splitkey, node** splitnode)
    {
            ...
            if (newchild)
            {
                ...
                if (inner->isfull())
                {
                    ...
                    if (slot == inner->slotuse+1 && inner->slotuse < (*splitnode)->slotuse)
                    {
                        ...
                        ASSIGN_CHILD(inner, inner->slotuse+1, splitinner->childid[0]);
                        inner->slotuse++;

                        // set new split key and move corresponding datum into right node
                        ASSIGN_CHILD(splitinner, 0, newchild);
                        *splitkey = newkey;

                        check_node(inner);
                        check_node(splitinner);
                ...
                ASSIGN_CHILD(inner, (slot + 1), newchild);
                inner->slotuse++;
        ...
        else // n->isleafnode() == true
        {
            ...
            if (leaf->isfull())
            {
                split_leaf_node(leaf, splitkey, splitnode, slot);
...
#ifdef ENABLE_FIXUP
                icb_->simple_insert(leaf, slot);
#endif
            }

            // put data item into correct data slot

            BTREE_PRINT("btree::insert_descend leaf" << leaf << " slot " << slot <<std::endl);
            globalLastLeafModified_ = leaf;
            int i = leaf->slotuse - 1;
            BTREE_ASSERT(i + 1 < leafslotmax);

            while(i >= 0 && key_less(ctx_, key, leaf->slotkey[i])) {
    ...
    void split_leaf_node(leaf_node* leaf, key_type* _newkey, node** _newleaf, unsigned short insertPoint)
    {
        ...
        leaf_node *newleaf = allocate_leaf(NULL);
        ...
        // handle iterator fixup if any
#ifdef ENABLE_FIXUP
        icb_->split(leaf, newleaf, insertPoint, mid);
#endif
    ...
    void split_inner_node(inner_node* inner, key_type* _newkey, node** _newinner, unsigned int addslot)
    {
        ...
        inner_node *newinner = allocate_inner(inner->level, NULL);
        ...
        for(unsigned int slot = mid + 1; slot < inner->slotuse; ++slot)
        {
            ...
            ASSIGN_CHILD(newinner, ni, inner->childid[slot]);
        }
        ASSIGN_CHILD(newinner, newinner->slotuse, inner->childid[inner->slotuse]);
...
public:
    ...
    // FIXME: shouldn't this iterator be a const reference?
...
#endif
        if (false && root == NULL) {
            // the following forces instantiation of these methods
            publishTreeSlice(iter);
            publishTreeSlice(iter.currnode);
            publishTreeSlice((inner_node*)iter.currnode);
            dump_node(0);
        }
    }
...
private:
    ...
    //
    //
    result_t erase_leaf(leaf_node* leaf, leaf_node* leftleaf, leaf_node* rightleaf,
            inner_node* leftparent, inner_node* rightparent,
            inner_node* parent, unsigned int slot, unsigned int parentslot)
    {
        globalLastLeafModified_ = leaf;
        ...
        // NOTE: we assume that it is only the root node whose slotuse can
        //       go to zero here.  Otherwise, we would have merged to get 
        //       slotuse up.
        ...
        if (slot == leaf->slotuse)
        {
            ...
            {
                BTREE_ASSERT(parent->childid[parentslot] == leaf);
        ...
        if (leaf->isunderflow() && !(leaf == root && leaf->slotuse >= 1))
        {
            ...
            else if ( (leftleaf == NULL || leftleaf->isfew()) && (rightleaf == NULL || rightleaf->isfew()) )
            {
                if (leftparent == parent) {
                    myres |= merge_leaves(leftleaf, leaf, leftparent, slot, 1);
                } else {
                    myres |= merge_leaves(leaf, rightleaf, rightparent, slot, 0);
                }
            }
            ...
            else if ( (leftleaf != NULL && leftleaf->isfew()) && (rightleaf != NULL && !rightleaf->isfew()) )
            {
                if (rightparent == parent) {
                    myres |= shift_left_leaf(leaf, rightleaf, rightparent, parentslot, slot);
                } else {
                    myres |= merge_leaves(leftleaf, leaf, leftparent, slot, 1);
                }
            }
            ...
            {
                if (leftparent == parent) {
                    shift_right_leaf(leftleaf, leaf, leftparent, parentslot - 1, slot);
                } else {
                    myres |= merge_leaves(leaf, rightleaf, rightparent, slot, 0);
                }
            }
            ...
            // handle iterator fixup for erase with no merges
#ifdef ENABLE_FIXUP
            icb_->simple_erase(leaf, slot);
#endif
        }
    ...
    result_t erase_inner(inner_node* inner, inner_node* leftinner, inner_node* rightinner,
                         inner_node* leftparent, inner_node* rightparent, inner_node* parent,
                         unsigned short slot, unsigned short parentslot, result_t result)
    {
        result_t myres = btree_ok;
        if (result.has(btree_update_lastkey))
        {
            if (parent && parentslot < parent->slotuse)
            {
                ...
                BTREE_ASSERT(parent->childid[parentslot] == inner);
        ...
        if (result.has(btree_fixmerge))
        {
            ...
            for(int i = slot; i < inner->slotuse; i++)
            {
                ...
                ASSIGN_CHILD(inner, i, (inner->childid[i + 1]));
            }
            inner->slotuse--;
        ...
        if (leftinner != NULL) {
            check_node(leftinner);
        }
        check_node(inner);
        if (rightinner != NULL) {
            check_node(rightinner);
        }
        return myres;
    }
                               ...
                               inner_node *parent, unsigned int parentslot)
    {
        if (curr->isleafnode())
        {
            ...
            return erase_leaf(leaf, leftleaf, rightleaf, leftparent, rightparent, parent, slot, parentslot);
        }
        else // !curr->isleafnode()
        {
            ...
            if (slot == 0) {
                myleft = (left == NULL) ? NULL : (static_cast<inner_node*>(left))->childid[left->slotuse];
            ...
            return erase_inner(inner, leftinner, rightinner, leftparent, rightparent,
                               parent, slot, parentslot, result);
        }
    }

    /// erase one entry given an iterator designating that entry.
    /// We start at the leaf node.  We adjust what must be adjusted
    /// at the leaf with an appropriate set of results.  We then 
    /// recursively ascend the tree adjusting as we go.  We stop
    /// when no more adjustment is required or we reach the top of the
    /// tree.
    uint16_t get_parent_slot(node* child)
    {
        inner_node* parent = static_cast<inner_node*>(child->parent);
        if (parent == NULL) {
            return 0;
        }
        for (int i = 0; i <= parent->slotuse; ++i) {
            if (parent->childid[i] == child) {
                return i;
            }
        }
        // parent does not point to child. 
        SHOULD_NEVER_HAPPEN("btree structure broken");
        return 0;
    }

    // find left node of an inner node
    //@param curr -- node whose left node we want
    //@parem slot -- slot in the parent of the node whose left node we want
    inner_node* find_left(node* curr, unsigned short slot) {
        if (curr == root) {
            return NULL;
        }
        inner_node* parent = curr->parent;
        if (slot == 0) {
            // must look up a level
            if (parent == root) {
                return NULL;
            }
            inner_node* tp = find_left(parent, get_parent_slot(parent));
            return tp ? static_cast<inner_node*>(tp->childid[tp->slotuse]) : NULL;
        } else {
            return static_cast<inner_node*>(parent->childid[slot-1]);
        }
    }

    // find right node of an inner node
    //@param curr -- node whose right node we want
    //@parem slot -- slot in the parent of the node whose right node we want
    inner_node* find_right(node* curr, unsigned short slot) {
        if (curr == root) {
            return NULL;
        }
        inner_node* parent = curr->parent;
        if (slot == parent->slotuse) {
            // must look up a level
            if (parent == root) {
                return NULL;
            }
            inner_node* tp = find_right(parent, get_parent_slot(parent));
            return tp ? static_cast<inner_node*>(tp->childid[0]) : NULL;
        } else {
            return static_cast<inner_node*>(parent->childid[slot + 1]);
        }
    }
    
    void erase_one_ascend(node *curr, unsigned short slot, result_t result)
    {
        inner_node* parent = curr->parent;
        unsigned short parentslot = curr == root ? 0 : get_parent_slot(curr); 

        if (curr->isleafnode())
        {
            leaf_node *leaf = static_cast<leaf_node*>(curr);
            leaf_node *leftleaf = leaf->prevleaf;
            inner_node* leftparent = leftleaf == NULL ? NULL : leftleaf->parent;
            leaf_node *rightleaf = leaf->nextleaf;
            inner_node* rightparent = rightleaf == NULL ? NULL : rightleaf->parent;
            BTREE_PRINT("erase_ascend: leaf " << leaf << " slot " << slot << " result " << result.flags <<  std::endl);

            if ((result = erase_leaf(leaf, leftleaf, rightleaf, leftparent,
                                 rightparent, leaf->parent, slot, parentslot)).flags == btree_ok) {
                return;
            }
         
            if (curr != root) {
                erase_one_ascend(parent, parentslot, result);
            }
        }
        else
        {
            inner_node *inner = static_cast<inner_node*>(curr);
            inner_node *leftinner = find_left(inner, parentslot);
            inner_node *rightinner = find_right(inner, parentslot);
            inner_node* leftparent = leftinner ? leftinner->parent : NULL;
            inner_node* rightparent = rightinner ? rightinner->parent : NULL;
            BTREE_PRINT("erase_ascend: inner " << inner << " slot " << slot << " result " << result.flags << std::endl);
            
            if ((result = erase_inner(inner, leftinner, rightinner, leftparent, rightparent,
                                  parent, slot, parentslot, result)).flags == btree_ok) {
                return;
            }
    ...
    // FIXME: shouldn't this iterator be a const reference?
    void erase_one(iterator iter)
    {
        leaf_node*     leaf = iter.currnode;
        unsigned short slot = iter.currslot;

        erase_one_ascend(leaf, slot, btree_ok);
        --stats.itemcount;
    }
    ...
    result_t merge_leaves(leaf_node* left, leaf_node* right, inner_node* parent,
                          unsigned short slot, unsigned short side)
    {
...
#ifdef ENABLE_FIXUP
        unsigned short leftSize = left->slotuse;
#endif
...
#ifdef ENABLE_FIXUP
        // handle iterator fixups as required
        if (side == 0) {
            icb_->merge_left(left, right, slot, leftSize);
        } else {
            icb_->merge_right(left, right, slot, leftSize);
        }
#endif
        check_node(left);
    ...
    /// removed by the calling parent node.
    result_t merge_inner(inner_node* left, inner_node* right, inner_node* parent, unsigned int parentslot)
    {
        ...
        {
            left->slotkey[left->slotuse + i] = right->slotkey[i];
            ASSIGN_CHILD(left, (left->slotuse + i), right->childid[i]);
        }
        left->slotuse += right->slotuse;

        ASSIGN_CHILD(left, left->slotuse, right->childid[right->slotuse]);
    ...
    /// if possible.
    /*static*/ result_t shift_left_leaf(leaf_node *left, leaf_node *right, inner_node *parent,
                                    unsigned int parentslot, unsigned short slot)
    {
        BTREE_ASSERT(left->isleafnode() && right->isleafnode());
        BTREE_ASSERT(parent->level == 1);
        ...
        check_node(left);
        check_node(right);
        // handle iterator fixup
#ifdef ENABLE_FIXUP
        icb_->shift_left(left, right, slot, leftSize, shiftnum);
#endif

        // fixup parent
        if (parentslot < parent->slotuse) {
            BTREE_PRINT("fix up parent after shift left " << parent << " slot " << parentslot << std::endl);
    ...
    void shift_left_inner(inner_node *left, inner_node *right, inner_node *parent, unsigned int parentslot)
    {
        ...
        ASSIGN_CHILD(left, left->slotuse, right->childid[shiftnum-1]);
    ...
    /// if possible.
    /*static*/ void shift_right_leaf(leaf_node *left, leaf_node *right, inner_node *parent, unsigned int parentslot, unsigned short insertPoint)
    {
        BTREE_ASSERT(left->isleafnode() && right->isleafnode());
        BTREE_ASSERT(parent->level == 1);
        ...
        // handle iterator fixup
#ifdef ENABLE_FIXUP
        icb_->shift_right(left, right, insertPoint, leftSize, shiftnum);
#endif
    ...
    /// right so that both nodes are equally filled. The parent node is updated
    /// if possible.
    void shift_right_inner(inner_node *left, inner_node *right, inner_node *parent, unsigned int parentslot)
    {
        BTREE_ASSERT(left->level == right->level);
        BTREE_ASSERT(parent->level == left->level + 1);
        ...
        ASSIGN_CHILD(right, shiftnum-1, left->childid[left->slotuse]);
        ...
        for(unsigned int i = 0; i < shiftnum - 1; i++)
        {
            ...
            ASSIGN_CHILD(right, i, (left->childid[left->slotuse - shiftnum + i + 1]));
        }
...
public:
    // *** Verification of B+ Tree Invariants
     ...
     void dump_node(const node* n) {
        ...
        printf("node @ %p; keys = ", n);
        if (n->isleafnode()) {
            const leaf_node *leaf = static_cast<const leaf_node*>(n);

            for(unsigned short slot = 0; slot < leaf->slotuse; ++slot) {
                //printf(" %s ", printKeyFunc_(ctx_, leaf->slotkey[slot]));
            }
        } else {
            ...
            for(unsigned short slot = 0; slot < inner->slotuse; ++slot) {
               //printf(" %s ", printKeyFunc_(ctx_, inner->slotkey[slot]));
            }
        }
    ...
    inline void check_node(node* n, bool checkParent = false) {
#if 0
        key_type key;
        dump_node(n);
        if (n->isleafnode()) {
            ...
            for(unsigned short slot = 0; slot < leaf->slotuse - 1; ++slot) {
                if (leaf->slotkey[slot] == leaf->slotkey[slot+1] ||
                     !key_lessequal(leaf->slotkey[slot], leaf->slotkey[slot + 1])) {
                    assert(false);
                    return;
                }
                key = leaf->slotkey[leaf->slotuse-1];
            }
        } else {
            ...
            for(unsigned short slot = 0; slot < inner->slotuse - 1; ++slot) {
                ...
                key = inner->slotkey[inner->slotuse-1];
            }
        }

        if (checkParent) {
            inner_node* parent = n->parent;
            if (n == root || parent == NULL) {
                return;
            }
            uint16_t slot = get_parent_slot(n);
            if (slot < parent->slotuse) {
                if (!key_lessequal(key, parent->slotkey[slot])) {
                    dump_node(parent);
                }
            }
        }
    ...
    void verify() const
    {
        ...
        if (root)
        {
            verify_node(root, &minkey, &maxkey, vstats, NULL);
...
private:
    ...
    void verify_node(const node* n, key_type* minkey, key_type* maxkey, tree_stats &vstats, const inner_node* parent) const
    {
        ...
        else // !n->isleafnode()
        {
            ...
            for(unsigned short slot = 0; slot <= inner->slotuse; ++slot)
            {
                ...
                verify_node(subnode, &subminkey, &submaxkey, vstats, inner);
...
public:
    ...
    // FIXME persistence code should be removed from the btree.  For DS, we
    //       do not persist index structures
...
private:
    ...
    /// serialization.
    // FIXME does not properly restore parent pointers
    node* restore_node(std::istream &is)
    {
        ...
        else
        {
            ...
            *newinner = nu.inner;
            ...
            {
                ASSIGN_CHILD(newinner, slot, restore_node(is));
            }
    ...
    /// walking support: used for maintenance
private:
    void poWalkerInternal(node* in, WalkerCallbackInterface& cb) {
        cb((void*)in);
        if (!in->isleafnode()) {
            inner_node* n = static_cast<inner_node*>(in);
            for (int slot = 0; slot <= n->slotuse; ++slot) {
                cb((void*)n->childid[slot]);
                poWalkerInternal(n->childid[slot], cb);
            }
        }
    }
public:
    void preorderWalker(WalkerCallbackInterface& cb) {
        if (root != NULL) {
            poWalkerInternal(root, cb);
        }
    }
