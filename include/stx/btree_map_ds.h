// $Id: btree_map.h 113 2008-09-07 15:25:51Z tb $
/** \file btree_map.h
...
public:
    ...
    inline btree_map(void * context, const key_compare &kcf)
        : tree(context, kcf)
    {
    }
    ...
    /// constructor initializing an empy B+ tree with special key
    /// comparison object and a context for that object
    inline btree_map(void * context, const key_compare &kcf, AbstractShard* shard)
        : tree(context, kcf, shard)
    {
    }


    /// Output out a key to a FILE
    typedef void (*IndexPrintKeyFunc)(FILE* f, const ShardContext& ctx, const key_type& key);
    /// constructor initializing an empy B+ tree with special key
    /// comparison object and a context for that object
    inline btree_map(void * context,
                     const key_compare &kcf,
                     AbstractShard* shard,
                     IndexPrintKeyFunc printKeyFunc)
        : tree(context, kcf, shard, printKeyFunc)
    {
    }
...
public:
    void* getGlobalLastLeafModified() {
        return tree.getGlobalLastLeafModified();
    }

    void setMonitor(bool value) {
        tree.setMonitor(value);
    }

    void publishTreeSlice(iterator& iter) {
        tree.publishTreeSlice(iter);
    }
...
public:
    ...
    /// identical key entries found. Since this is a unique map, count() returns either 0 or 1.
